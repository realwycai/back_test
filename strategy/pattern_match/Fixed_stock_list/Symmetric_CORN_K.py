...
def fetch_optimal_portfolio(datechar, W, Pho, k, price_daily_wide, correlation):
    ...
    for w in W:
        ...
        for p in Pho:
            similar_period_positive = correlation_coefficient[correlation_coefficient.coefficient > p].trade_dt
            similar_period_negative = correlation_coefficient[correlation_coefficient.coefficient < -p].trade_dt
            if similar_period_positive.empty:
                continue
            if similar_period_negative.empty:
                continue
            ...
            res = minimize(lambda b: -np.log(np.dot(b.T, np.mat(C_x_positive))).sum()+np.log(np.dot(b.T, np.mat(C_x_negative))).sum(), b0, constraints=cons)
